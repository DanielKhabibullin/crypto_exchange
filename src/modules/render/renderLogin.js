...
export const renderLogin2 = () => {
	...
	mount(main, auth);
	const loginErrorEl = auth.querySelector(`
		.auth__form-input-wrapper:nth-child(1) .auth__form-error`);
	const loginInputEl = auth.querySelector('.auth__form-input[name="login"]');
	const passwordErrorEl = auth.querySelector(`
		.auth__form-input-wrapper:nth-child(2) .auth__form-error`);
	const passwordInputEl = auth.querySelector(`
		.auth__form-input[name="password"]`);
